...
enum RelationType { SameService = 12, VehicleGroupDuration = 11, ForceLast = 10, ForceFirst = 9, NeverFirst = 8, MaximumDurationLapse = 7, MeetUp = 6, Shipment = 5, MaximumDayLapse = 4, MinimumDayLapse = 3, SameRoute = 2, Order = 1, Sequence = 0 };
...
void TSPTWDataDT::LoadInstance(const std::string & filename) {
  ...
  for (const ortools_vrp::Relation& relation: problem.relations()) {
    ...
    else if (relation.type() == "same_service") type = SameService;
